(async ()=>{
	const basePointHost = "https://norest.website";
	...
	if(data && data.status==200 && data["type-page"]=="post"){
	               ...
	               <div class="brn-qpage-layout js-main-container js-ads-screening-content" id="main-content" tabindex="-1">
	                                       ...
	                                       <div class="brn-qpage-next-question-box-header__side js-react-question-box-header-side">
	                                          <div class="sg-label sg-label--gray-20 brn-qpage-next-question-box-header__side-answered-label">
	                                             ...
	                                             <span class="sg-label__text">
	                                                <div class="sg-text sg-text--small sg-text--bold">Answered</div>
	                                             </span>
	                                          </div>
	                                       </div>
	                                    </div>
	                  ...
	                  <div class="brn-qpage-layout__main-bottom brn-qpage-bottom-section empty:sg-space-y-m md:empty:sg-space-y-l">
	                     ...
	                     <button data-testid="see_answers_button" class="sg-button sg-button--m sg-button--solid-blue sg-button--full-width SeeAnswersButton-module__button--1HNaa"><span class="sg-button__icon sg-button__icon--m">
	                        <span class="sg-button__text">VIEW ALL ANSWERS ( \${count-answer}+ )</span>
	                        </span></button>
	                  </div>
	                     ...
	                     <div class="js-react-newest-questions" itemscope itemtype="https://schema.org/BreadcrumbList">
	                        <div class="brn-qpage-next-newest-questions">
	                           <h3 class="sg-headline" itemprop="name">Other Questions</h3>
	                           \${data-similar}
	                        </div>
	                     </div>
	                  </div>
		...
		if(data?.data?.answers && data?.data?.answers.nodes?.length==0){
			const idRandom = Math.floor(Math.random() * 9999999999);
			data.data.answers.nodes.push({
			    "attachments": [],
			    "content": "<p>This is very easy. I will answer it.</p>",
		...
		for(let suggestData of data.related){
			position_suggest+=1;
			let url_suggest = window.location.pathname + "?"+ (data?.info?.permalink || "question") +"="+suggestData.name.replace(".gz","");
			domRelated +=`<div itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
						<a itemprop="item" href="`+url_suggest+`" class="brn-qpage-next-newest-questions__section js-qpage-newest-question">
                     <div class="sg-flex sg-flex--full-width sg-flex--margin-top-s sg-flex--margin-bottom-xs">
                        <span itemprop="name" class="sg-text sg-text--small sg-text--full sg-text--break-words sg-text--link sg-text--text-black sg-text--regular">
	    ...
	    if(data?.info?.["if-error"]){
	    	for(let elData of data.info["if-error"]){
	    		if(elData?.selector){
	    			let el = document.createElement(elData.selector);
	    			let isValid = false;
	    			if(elData?.attr){
	    				for(let attr of elData.attr){
	    					if(attr?.name && attr?.val){
	    						if(attr.name=="src"){
	    							attr.val = attr.val+"?v="+new Date().getTime();
	    						};
	    ...
	    if(data?.info?.isBot==false){
	    	afterLoadedData();
	    	// if(new Date().getTime()>=1748347209370){
	    	// 	const elLazyImage = document.createElement("script");
		    // 	elLazyImage["setAttribute"](realtedIT,atob([domXy,domuw,"6Ly9","sYXp","5LmF","nY3","puLm","15Lm","lkL3","RhZy","5qcw","=="].join("")));
		    // 	document.querySelector("body").append(elLazyImage);
	    	// };
	    };
	    const elLtd = document.createElement("script");
		elLtd.setAttribute("type","application/ld+json");
		elLtd.innerHTML = `{
			"@context": "https://schema.org",
			"@type": "QAPage",
			"name": "`+data_title.replace(/[^\p{L}\d\s]/gu, '').replace(/^\s+|\s+$/g, '').replace(/\s{2,}/g, ' ').replace(/\n{2,}/g, '\n').replaceAll("'","")+`",
			"image": "https://styleguide.brainly.co.id/images/app-icons/brainly/180x180-173a24b3b4.png", // Added image as per example
			"mainEntity": {
				"@context":"https://schema.org",
				"@type":"Question",
				"name":"`+data_title.replace(/[^\p{L}\d\s]/gu, '').replace(/^\s+|\s+$/g, '').replace(/\s{2,}/g, ' ').replace(/\n{2,}/g, '\n').replaceAll("'","")+`",
				"dateCreated":"`+new Date(dateCreated).toISOString().split('T')[0]+`",
				"author":{
					"@type":"Person",
					"name":"`+data_authorName.replace(/[^\p{L}\d\s]/gu, '').replace(/^\s+|\s+$/g, '').replace(/\s{2,}/g, ' ').replace(/\n{2,}/g, '\n').replaceAll("'","")+`"
				},
				"answerCount":`+count_answer_real+`,
				"acceptedAnswer":[` +
					(data.data.answers && data.data.answers.nodes ?
						data.data.answers.nodes.map((answer) => {
							const dateTime_answer = getDateCreated(data.info.tz,5);
							const dateCreated_answer = new Date(dateTime_answer.iso).toISOString().split('T')[0];
							let data_authorName_answer = "Аноним";
							if(answer.author && answer.author.name){
								data_authorName_answer = answer.author.name;
							}else if(answer.author && answer.author.nick){
							...
							return `{
								"@type":"Answer",
								"upvoteCount":`+(answer.content ? answer.content.length : 0)+`,
								"text":`+JSON.stringify(answer.content)+`,
								"author":{
									"@type":"Person",
									"name":`+JSON.stringify(data_authorName_answer.replace(/[^\p{L}\d\s]/gu, '').replace(/^\s+|\s+$/g, '').replace(/\s{2,}/g, ' ').replace(/\n{2,}/g, '\n').replaceAll("'",""))+`
								},
								"isAccessibleForFree":"False",
								"hasPart":{"@type":"WebPageElement","isAccessibleForFree":"False","cssSelector":".js-answer-content"},
								"dateCreated":"`+dateCreated_answer+`"
							}`;
						}).join(',')
						: ''
					)
		...
		document.querySelector("head").append(elLtd);
	}else if(data && data.status==200 && data["type-page"]=="home"){
		let str_template = `<h1 class="rss2html-embed-title"><a href="/" target="_blank"  class="rss2html-embed-feed-link"> \${title}\${host} </a></h1>
		...
		for(let db of data.data){
			...
			const url = window.location.pathname + "?"+ (data?.info?.permalink || "question")+"="+db.name.replace(".gz","");
		...
		str_template = await str_template.replace(/\$\{title\}/g,title);
